...
const ProblemDescription = () => {
  ...
  useEffect(() => {
    const fetchProblem = async () => {
      try {
        // Using problemsApi — clean and simple!
    ...
    <section className="w-1/3 border-r border-slate-800 flex flex-col bg-slate-900/20">
      <div className="p-6 overflow-y-auto flex-1 custom-scrollbar">
        <div className="flex items-center gap-3 mb-4">
          <span className="px-2.5 py-1 text-xs font-bold rounded-md bg-yellow-500/10 text-yellow-500 border border-yellow-500/20 uppercase tracking-wider">{problem.difficulty}</span>
          <span className="text-slate-500 text-sm">•</span>
          {/* <span className="text-slate-400 text-sm italic">Time Limit: {problem && problem.timeLimit}</span> */}
        </div>
        <h2 className="text-3xl font-bold text-white mb-6 tracking-tight">{problem && problem.title}</h2>
        <div className="prose prose-invert max-w-none space-y-4 text-slate-300">
          {problem && problem.description}
          <div className="mt-8 space-y-6">
            {problem && problem.sampleTestCases.map((example, index) => (
              <div key={index}>
